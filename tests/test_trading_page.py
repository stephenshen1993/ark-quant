...
class TestTradingPageInteraction(unittest.TestCase):
    ...
    def test_market_temperature_is_a_shared_plan_input_not_account_fact(self):
        self.assertIn("class=\"plan-masthead-temperature\"", self.html)
        self.assertIn('aria-label="当前全市场温度"', self.html)
    ...
    def test_account_trading_plans_use_real_accounts_and_complete_cash_equation(self):
        ...
        self.assertIn("accountFundingSummary(plan)", self.html)
        ...
        self.assertIn("资金估算过程", self.html)
    ...
    def test_expanded_account_plan_keeps_context_and_execution_guardrails(self):
        self.assertIn('class="plan-account-folio"', self.html)
        ...
        self.assertIn("上一个账户", self.html)
        self.assertIn("下一个账户", self.html)
        trade_table = self.html.index('class="plan-trade-section"')
        guardrails = self.html.index('class="plan-execution-guardrails"', trade_table)
        cash_audit = self.html.index('class="plan-cash-audit"', guardrails)
        self.assertLess(trade_table, guardrails)
        self.assertLess(guardrails, cash_audit)
    ...
    def test_account_plan_details_render_one_execution_table_with_action_as_the_only_trade_semantic(self):
        ...
        self.assertNotIn('x-for="order in group.orders"', self.html)
        self.assertNotIn('class="plan-trade-action-group-row"', self.html)
        self.assertNotIn("group.orders.length + ' 笔'", self.html)
    ...
    def test_account_plan_summary_exposes_the_trade_detail_toggle(self):
        ...
        self.assertIn('class="plan-account-open-summary"', self.html)
        self.assertIn("accountExecutionSummary(plan)", self.html)
        self.assertIn("需先调入", self.html)
        self.assertIn("无需调入", self.html)
        self.assertIn("${group.label} ${group.orders.length} 笔", self.html)
        self.assertIn("计划后资金", self.html)
        self.assertIn("accountPlanFolioTitle(plan)", self.html)
        self.assertIn("display_ordinal: `${index + 1}/${plans.length}`", self.html)
    ...
    def test_new_account_trade_rows_remain_compact_mobile_records(self):
        ...
        self.assertIn('"code name action side amount"', self.html)
        self.assertIn('"position position position price amount"', self.html)
    ...
    def test_plan_page_uses_server_states_and_puts_actions_before_readiness(self):
        ...
        self.assertIn('class="plan-masthead-revision"', self.html)
